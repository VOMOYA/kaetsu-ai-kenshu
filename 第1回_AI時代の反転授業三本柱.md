...
> [!quote] 田原氏の言葉
> **「質問される前には答えは存在していなかった。質問されることで答えが初め作られて存在してくる」**

#### ChatGPT との類比

```mermaid
graph TD
    A["ChatGPT内部<br/>混沌とした情報の絡まり<br/>（答えはまだない）"] -->|"質問という行為"| B["情報が集まり<br/>生成される"]
    B --> C["『答え』が初めて存在する"]
...
flowchart TD
    A["① 場を整える<br/>チェックイン"] --> B
    B["② 問いかけ<br/>→ 個別の声が生成される"] --> C
    C["③ 声を重ね合わせ<br/>→ 干渉縞（パターン）が現れる"] --> D
    D["④ 縞模様を見ながら対話<br/>→ 気づきが深まる"] --> A
...
graph LR
    subgraph pre["事前学習フェーズ"]
        PA["事前学習AIアプリ<br/>🔬 回折格子"] -->|"問いかけで声を引き出す"| PB["生徒の個別の声・考え"]
    end
    subgraph int["統合フェーズ"]
        IA["統合AIアプリ<br/>📊 干渉縞検出スクリーン"] -->|"個別の声を重ね合わせ"| IB["パターンの可視化"]
    end
    PB --> IA
...
```mermaid
graph TD
    A["講義<br/>（田原氏）"] --> B["ブレイクアウト<br/>3人×4G / 15分"]
    B --> C["全体共有<br/>09:50〜09:55"]
    C --> D["次のパート<br/>AIから情報を引き出す3つの方法"]
...
```mermaid
graph TD
    A["① 一般情報<br/>AIのデータベース"] --> D
    B["② 役割・ロール<br/>情報の調理方法"] --> D
    C["③ 個別情報<br/>自分だけが持つデータ"] --> D
    D["推論エンジン"] --> E["🎯 アウトプット生成"]
...
```mermaid
flowchart TD
    S1["STEP 1<br/>アプリのアイデアをAIと対話して固める<br/>「こんなことをやりたい」をざっくり話す"] --> S2
    S2["STEP 2<br/>3要素を対話的に明確にする<br/>② 専門ロール ／ ③ 個別情報（知識）<br/>／ コミュニケーション設計"] --> S3
    S3["STEP 3<br/>システムプロンプトをAIに生成させる<br/>「このGemのシステムプロンプトを作って」"] --> S4
    S4["STEP 4<br/>GeminiでGemを作成する<br/>左メニュー「Gem」→「新しいGemを作成」<br/>名前・説明入力 →「カスタム指示」に貼り付け"] --> S5
    S5["STEP 5<br/>知識ファイルをアップロード（任意）<br/>「知識」欄に講義要約PDF等をアップ"] --> S6
    S6["STEP 6<br/>テストプレイして動作確認 → 完成・共有"]
...
```mermaid
graph LR
    A["省察的実践<br/>Reflective Practice<br/>🪞 鏡モデル"] -->|"パラダイムシフト"| B["回折的実践<br/>Diffractive Practice<br/>🌊 干渉縞モデル"]

    C["AI = エージェンシャル・<br/>リアリズムのモデル"] --> B
    D["AI × 回折的実践<br/>相性が良い"] --> B
